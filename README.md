...
1. **按两次 `S` 键 呼出校验数字的正则表达式**
2. **按两次 `Z` 键 呼出校验字符的正则表达式**
3. **按两次 `T` 键 呼出特殊需求的正则表达式**
...
由英文、数字、汉字组成且字符长度在 m 到 n 之间

```javascript
/^[\u4E00-\u9FA5A-Za-z0-9]{m,n}$/;
```

由英文开头且由英文、数字、下划线组成且字符长度在 m 到 n 之间

```javascript
/^[A-Za-z][A-Za-z0-9_]{m,n}$/;
```

由英文、数字、特殊字符组成且至少包括 1 个大写字母、1 个小写字母、1 个数字、1 个特殊字符且字符长度在 m 到 n 之间
